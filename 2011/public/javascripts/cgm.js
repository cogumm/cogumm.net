...
$('form#contact').submit(function(){
  ...
  var bt = $('#contact button')
  bt.html(bt.attr('data-wait'))
  $.post(url, data, function(result){
    //$('#send_button').attr('disabled', true)
    $('#contact').find('input, textarea, button').attr('disabled', false)
    if(result['ok']){
...
var $top1= $('#cgm_description').offset().top + 100;
...
$("a[href^='#']").click(function(){
  ...
  pos.top -= 40
  $.scrollTo(pos,600)
  if(href == '#contact') $('.send_another').click()
  if(href == '#top') change_emphasis(emphasis, 0)
  return false
})
emphasis = [".web_two",".ruby_on_rails",".postgresql", ".mysql",".open_source",".methods"]
next_emphasis = null
